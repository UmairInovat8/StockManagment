...
async function run() {
    try {
        ...
        for (let i = 0; i < 2; i++) {
            itemValues.push([
                randomUUID(), "BATCH_TEST_" + i, "Batch Item", "Brand", null, null, null, null, null, null, 0, "EA", null, null, "{}", "9611ad9b-8c1c-4e89-8d7d-5c1cf7da32f9", null, new Date().toISOString(), new Date().toISOString()
            ]);
        }
